...
query = {'type': 'select', 'aggregate': None, 'agg_columns': None, 'columns': ['batsman'], 'clauses': {'and': ['wickets = 5', 'runs = 29'], 'or': []}}
...
def get_column_num(query_cols, schema_cols, aggregate, query_and):    
    if aggregate == 0: #Get what column numbers SELECT will take
        ...
        return get_col_nums
    if aggregate == 1: #Get what column numbers aggregate will take
        a = int(schema_cols.index(query_agg))
        get_col_nums_list1.append(a)
        get_col_nums1.update({'aggregate' : a})
        return get_col_nums1
    if aggregate == 2: #Get what column numbers and their values WHERE will take
        for i in range(len(query_and)): 
            want = [query_and[i].split(' ') for i in range(len(query_and))]
